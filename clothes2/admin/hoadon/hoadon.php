        ...
        public function show_hoadonPhanTrang()
        {
           ...
           if (isset($_GET['wordSearch']) && !empty($_GET['wordSearch']) ){
                if (isset($_GET['timTheo']) && !empty($_GET['timTheo']) ){
                    ...
                    $query = "SELECT * FROM cart,user WHERE (cart.id_khachhang = user.id) AND cart_status!=3 ORDER BY cart.id DESC LIMIT  $tungTrang, $sanPhamTungTrang "; //DESC: sản phẩm mới nhất sẽ lên đầu danh sách
            }
